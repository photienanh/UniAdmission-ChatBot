...
def generate_search_keywords(question: str, model: str = "gpt-4o-mini") -> Dict[str, Union[str, List]]:
    """
    Generate search keywords for Kaggle model using the same logic as the app.
    Returns either web search keywords or vector DB search parameters.
    """
    # Check API key
    gpt_api_key = os.getenv("GPT_API_KEY")
    if not gpt_api_key:
...
def route_search(question: str) -> Dict[str, Union[str, List]]:
    """
    Route search based on question analysis - decides between web search or vector DB
    """
    try:
        ...
        print(f"[KeywordGenerator] Error in route_search: {e}")
        # Fallback
        return {"type_search": "web_search", "key_word": [question]}
