...
class VLLMEngine:
    ...
    def _available(self, info: VLLMJobInfo):
        ...
        return False
    async def _process_request(self, info: VLLMJobInfo) -> VLLMJobResult:
        ...
        async with ClientSession() as session:
            async with session.post(f"{url}/generate", json=payload) as response:
                if response.ok:
                    ...
                    return VLLMJobResult(**data)
                else:
                    err_text = await response.text()
                    print(f"[VLLM Engine] Error {response.status}: {err_text}")
                    return VLLMJobResult(text=[err_text])
    async def process(self, info: VLLMJobInfo) -> VLLMJobResult:
